...
> ⚠️ **Этот документ отстал.** Последняя правка — 27 июля 2026, то есть до
> августовской кампании по многошаговости. Он описывает состояние с пресетами
> `orthrus`, `flowdraft_packed_blockwise` и `smollm_masked_baseline`, которые
> с тех пор вынесены в `bucket/`, и его числа перекрыты более поздним замером.
> Актуальны [README.md](README.md) и [EXPERIMENTS.md](EXPERIMENTS.md).
