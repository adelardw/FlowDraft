...
При заданном `checkpoint` оценка восстанавливает сохранённые конфиг модели,
вариант и все обучаемые DF-параметры. Отсутствующие, неизвестные или
несовместимые по форме тензоры приводят к ошибке до запуска benchmark'а;
device, dtype и attention-kernel остаются настройками eval-конфига.
...
выход побитово равен AR и NLL не измеряет ничего).

Таблица качества Orthrus охватывает пять семейств: **GSM8K**,
**MATH-500**, **AIME**, **HumanEval** и **MBPP**. AIME разделён на наборы
2024 и 2025 годов, поэтому запускаемых конфигов шесть: `gsm8k`, `math500`,
`aime24`, `aime25`, `humaneval`, `mbpp`. В расширенной efficiency-таблице
статьи также есть Pseudo2Code и LiveCodeBench-v5.
...
Каждый промпт декодируется выбранным драфтером и чистым AR; перед расчётом
acceptance, TPF и throughput проверяется побитовое совпадение. Поэтому
качество в точности наследуется от замороженного AR; pass rate для
HumanEval/MBPP считается их официальными sandboxed execution-harness'ами.
`data=nemotron` остаётся для оценки на обучающем распределении.
Сэмплирующая оценка
...
## Валидация против официального релиза Orthrus

Наш baseline Orthrus может быть неверен двумя независимыми способами: может быть
сломан цикл декодирования, а может быть плохим обучение. Бенчмарк одного только
нашего чекпоинта эти причины не различает. Поэтому мы независимо меняем **веса**
и **код** и измеряем все четыре комбинации против релиза авторов
([`chiennv/Orthrus-Qwen3-1.7B`](https://huggingface.co/chiennv/Orthrus-Qwen3-1.7B), CC-BY-4.0):

| ячейка | веса диффузии | цикл декодирования | точка входа |
| --- | --- | --- | --- |
| A | авторов | авторов | `src/eval_reference.py` на релизе |
| B | авторов | наш | `src/eval.py` на сконвертированном чекпоинте |
| C | наши | наш | `src/eval.py` на нашем обученном baseline |
| D | наши | авторов | `src/eval_reference.py` на обратно сконвертированном репозитории |

A против B изолирует нашу реализацию; A против C — наше обучение.

```bash
./scripts/run_validation.sh                 # профиль под одну GPU
PROFILE=full ./scripts/run_validation.sh    # протокол статьи: полные сплиты, 2048 новых токенов
DRY_RUN=1 ./scripts/run_validation.sh       # напечатать план запусков и выйти
```

Одна команда делает всё: скачивает релиз, если его нет, конвертирует веса в обе
стороны, прогоняет gate эквивалентности, обходит сетку, проверяет lossless и
пишет [results/validation.md](results/validation.md). Ничего не привязано к
железу: переопределите `PROFILE`, `BENCHMARKS`, `BLOCK_SIZES`, `DTYPE`, `ATTN`
или `PYTHON`, чтобы перенести прогон на другую машину.

**Докат после обрыва.** Все стадии resume-safe с гранулярностью **целого
прогона**. Перед каждым прогоном раннер ищет в файле результатов строку с той же
пятёркой `(run_id, codebase, weights_source, dataset, block_size)` и пропускает
его; запись идёт через `fcntl.flock`, а строка, оборванная убийством процесса,
игнорируется, а не роняет скан. Поэтому прерванная сетка продолжается с места
остановки, а завершённая перезапускается вхолостую — но прогон, который шёл в
момент падения, переигрывается с первого промпта, так как per-prompt строки
пишутся одним батчем в конце прогона. В `PROFILE=quick` это ~3 минуты; в
`PROFILE=full`, где один прогон — это целый сплит по 2048 токенов, это часы.

`run_id` — это профиль, и каждый профиль пишет собственный
`results/validation-$PROFILE.jsonl`. И то и другое сделано намеренно: `quick` и
`full` отличаются только `n_prompts`/`max_new_tokens`, поэтому без этого
quick-сетка была бы принята за завершённую full-сетку и пропущена, а строки
обеих попали бы в одну таблицу как будто они сравнимы.

    DRY_RUN=1 ./scripts/run_validation.sh   # что уже сделано, а что ещё предстоит

**Почему конвертация точна.** Релиз хранит диффузионный путь как плотные
послойные близнецы (`...self_attn.{q,k,v,o}_proj_diff.weight`,
`{q,k}_norm_diff.weight`) — ровно те модули, которые выбирает наш
`adapter.w_names`: 6 на слой × 28 слоёв = 168 тензоров идентичной формы
(352.3M параметров в обоих случаях). `src/tools/convert_orthrus.py` читает
индекс из `adapter._df_names`, а не восстанавливает его арифметикой, а
подкоманда `verify` прогоняет релиз через нашу раскладку и обратно, требуя
побитового совпадения всех 169 тензоров. Поля `r`/`lora_alpha` в их
`config.json` рудиментарны — LoRA-тензоров в релизе нет. Их замороженный
backbone побитово равен стоковому `Qwen/Qwen3-1.7B`, так что обе ячейки
верифицируются одной и той же моделью. Единственное структурное отличие — вход
`[MASK]`: у них это словарный токен (`mask_token_id`), у нас — свободный
параметр `mask_embedding`; конвертер переносит этот вектор в обе стороны.

**Gate.** `src/tools/equivalence_gate.py` загружает веса авторов в оба кода,
даёт обоим одинаковый префиксный кэш и одинаковый блок
`[anchor, MASK × (K-1)]` и сравнивает один диффузионный проход в float32/eager.
Раннер считает провал фатальным: если эти два пути расходятся, сетка меряет баг,
а не драфтер.

**Как читать числа.** `acceptance` (принятых черновых токенов за цикл; чистый
anchor не считается, поэтому при K=32 максимум 31) и `TPF` — свойства метода и
сравнимы **между** кодовыми базами. `tokens_per_s` и `speedup` дополнительно
несут накладные расходы Python каждого harness'а — AR-бейзлайн у авторов это HF
`GenerationMixin.generate`, у нас — рукописный цикл, — поэтому они сравнимы
только **внутри** одной кодовой базы, против её собственного AR. Сетка
пропускной способности идёт в bf16 с fused-ядрами, где побитовое равенство с AR
ломается по чисто численным причинам; поэтому lossless сертифицируется отдельным
аудитом в float32/eager, а не ассертится посреди сетки.
