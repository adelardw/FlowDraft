...
./hf-auth.sh uv run python src/eval.py model=qwen2_0.5b variant=orthrus checkpoint=<ckpt>
# (iii) flow-map, 1 прыжок (основной замер)
./hf-auth.sh uv run python src/eval.py model=qwen2_0.5b variant=flowdraft checkpoint=<ckpt>
...
./hf-auth.sh uv run python src/eval.py model=qwen2_0.5b variant=flowdraft checkpoint=<ckpt> data=nemotron
```
