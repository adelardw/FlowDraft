...
## Объём кода в токенах

Строки — метрика, зависящая от форматирования: одно выражение можно разбить
на две строки, длинное имя не увеличивает содержание. Ниже подсчёт по токенам
Python на HEAD, с исключением комментариев и докстрингов.

| Автор | Токенов кода | Доля |
|---|---:|---:|
| V. Tekaev | 13 551 | 60.2% |
| Ya. Sergaev | 8 964 | 39.8% |

Соотношение то же, что по строкам. **Метрики объёма не подтверждают приоритет
Ya. Sergaev на текущем состоянии кода и не должны использоваться как аргумент
о вкладе** — их легко пересчитать, и они измеряют, кто правил последним, а не
кто спроектировал. Проверяемые утверждения о вкладе — это создание файлов с
учётом переименований, авторство экспериментальной матрицы и датированные
находки, перечисленные выше и в хронологии.
